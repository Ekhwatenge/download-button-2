document.addEventListener("DOMContentLoaded",function(){
	let download = document.getElementById("upload");

	if (upload) {
		let progress = 0,
			strokeLen = 310,
			startDownload = function(progressRect,elText) {
				progress = 0;
				...
				this.disabled = true;
				this.classList.remove("download-btn-ready");
				this.classList.add("download-btn-running");
			...
			incProgress = function(progressRect,elText){
				...
				if (progress < 1) {
					...
					let timeout = 1500;
					setTimeout(resetDownload.bind(this,progressRect,elText),timeout);
				}
			},
			resetUpload = function(progressRect,elText) {
				this.classList.remove("download-btn-done");
				this.classList.add("download-btn-ready");
				this.disabled = false;
		...
		download.addEventListener("click",startDownload.bind(
			download,
